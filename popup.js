              ...
              if (numVal !== 0) {
                  ...
                  variationHtml = `
                    <div class="flex items-center gap-2">
                        <span class="flex items-center justify-center w-6 h-6 rounded-full ${escapeHTML(colorClass)}">
                            <span class="material-symbols-outlined text-[15px] ${escapeHTML(textClass)}">${escapeHTML(arrowIcon)}</span>
                        </span>
                        <div class="flex flex-col justify-center">
                            <span class="${escapeHTML(textClass)} text-[11.5px] font-bold leading-none">${escapeHTML(Math.abs(numVal).toFixed(2))}%</span>
...
function updateRealRateMonitor(targetRateSerie, inflationSerie, cetesSerie, tiieSerie) {
  ...
  if (tiieSerie && typeof tiieSerie.val === 'string' && tiieSerie.val !== "—") {
    ...
    if (spreadEl) {
      ...
      spreadEl.innerHTML = `TIIE vs Objetivo: ${spread > 0 ? "+" : ""}${escapeHTML(spread.toFixed(2))}%${warning}`;
    }
  }
    ...
    upcoming.forEach(ev => {
      ...
      item.innerHTML = `
          <div class="flex items-start justify-between w-full mb-1">
            ...
            <span class="${escapeHTML(impactClass)} shrink-0 mt-0.5">${escapeHTML(impactLabel)}</span>
          </div>
